...
public class MineSweapPart extends JFrame
{
	...
	private static final Color CELL_EXPOSED_FOREGROUND_COLOR_MAP[] = {Color.lightGray, Color.blue, Color.green, Color.cyan, Color.yellow,Color.orange, Color.pink, Color.magenta,Color.red,Color.red}; 
	...
	private void setMines() {
		for(int i=0; i<TOTAL_MINES; i++) {
			...
			if(!(mineGrid[randRow][randCol]==9)) {
				mineGrid[randRow][randCol]=9;
				if(randRow +1 < mineGrid.length) {
					if(mineGrid[randRow +1][randCol] != 9) {
					 ...
					 if (randCol + 1 < mineGrid[randRow + 1].length && mineGrid[randRow + 1][randCol + 1] != 9) {
					  ...
					  if (randCol - 1 >= 0 && mineGrid[randRow + 1][randCol - 1] != 9) {
	                        mineGrid[randRow + 1][randCol - 1]++;
	                    }
			}
				   if (randRow - 1 >= 0) {
				    ...
				    if (randCol - 1 >= 0 && mineGrid[randRow][randCol - 1] != 9) {
	                    mineGrid[randRow][randCol - 1]++;
	                }
			}
			else {
	...
	private String getGridValueStr(int row, int col) { 
		...
		else if (this.mineGrid[row][col] > NO_MINES_IN_PERIMETER_GRID_VALUE && this.mineGrid[row][col] <= ALL_MINES_IN_PERIMETER_GRID_VALUE ) 
	...
	else if ( flagged && !exposed && (mod & ActionEvent.ALT_MASK) != 0 ) 
	{
		...
		setTitle("MineSweap" + MineSweapPart.guessedMinesLeft +" Mines left");
		if(MineSweapPart.actualMinesLeft == 0) {
			setTitle("MineSweap you win, there are 0 Mines left");
		}
		}       
		...
		{      
			...
			if ( mineGrid[mjb.ROW][mjb.COL] == IS_A_MINE_IN_GRID_VALUE )    
			{          
				JOptionPane.showMessageDialog(null,"YOU LOSE :( "+ " YOU FOUND "+(16-MineSweapPart.actualMinesLeft + " MINES"));
				running = false;
				// what else do you need to adjust?       
				// could the game be over?       
